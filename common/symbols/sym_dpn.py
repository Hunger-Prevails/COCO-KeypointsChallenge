...
def DualPathFactory(data, num_1x1_a, num_3x3_b, num_1x1_c, name, inc, G, _type='normal', dilate=1, inc_dilate=False):
    kw = 3
    kh = 3
    pw = ((kw - 1) * dilate + 1) // 2
    ph = ((kh - 1) * dilate + 1) // 2
...
def get_symbol(data, inc_dilates):
    # define Dual Path Network
    input_dict = {'data': (2, 3, 352, 252)}
    ...
    inc = inc_sec[2]
    R = (k_R*bw)/256
    conv2_x_x, dilate = DualPathFactory(conv1_x_x, R, R, bw, 'conv2_x__1', inc, G, 'proj', dilate=dilate, inc_dilate=inc_dilates[0])
    ...
    inc = inc_sec[3]
    R = (k_R*bw)/256
    conv3_x_x, dilate = DualPathFactory(conv2_x_x, R, R, bw, 'conv3_x__1', inc, G, 'down', dilate=dilate, inc_dilate=inc_dilates[1])
    ...
    inc = inc_sec[4]
    R = (k_R*bw)/256
    conv4_x_x, dilate = DualPathFactory(conv3_x_x, R, R, bw, 'conv4_x__1', inc, G, 'down', dilate=dilate, inc_dilate=inc_dilates[2])
    ...
    inc = inc_sec[5]
    R = (k_R*bw)/256
    conv5_x_x, dilate = DualPathFactory(conv4_x_x, R, R, bw, 'conv5_x__1', inc, G, 'down', dilate=dilate, inc_dilate=inc_dilates[3])
    ...
    return [conv2_x_x[0], conv3_x_x[0], conv4_x_x[0], conv5_x_x[0]]
