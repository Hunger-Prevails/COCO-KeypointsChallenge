...
class SoftmaxAccMetric(mx.metric.EvalMetric):
    def __init__(self, config, metric_name='SoftmaxAcc', axis=1):
...
class SoftmaxAccFGMetric(mx.metric.EvalMetric):
    def __init__(self, config, metric_name='SoftmaxAccFG', axis=1):
        super(SoftmaxAccFGMetric, self).__init__(metric_name)
        self._axis = int(axis)
        self._pred_names = config.pred_names
        self._label_names = config.label_names
        self._suffix = str(config.count)
    ...
    def update(self, labels, preds):
        ...
        label = label.reshape((-1,)).astype(np.int32)
        pred = pred.argmax(axis).reshape((-1,)).astype(np.int32)
