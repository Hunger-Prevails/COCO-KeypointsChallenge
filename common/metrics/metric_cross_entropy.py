...
class CrossEntropyAccMetric(mx.metric.EvalMetric):
    def __init__(self, config, metric_name='CrossEntropyAcc', binary_thresh=0.5):
    ...
    def update(self, labels, preds):
        ...
        pred_name = 'cross_entropy_pred_' + self._suffix
...
class CrossEntropyAccFGMetric(mx.metric.EvalMetric):
    def __init__(self, config, metric_name='CrossEntropyAccFG', binary_thresh=0.5):
        super(CrossEntropyAccFGMetric, self).__init__(metric_name)
        self._binary_thresh = float(binary_thresh)
        self._pred_names = config.pred_names
        self._label_names = config.label_names
        self._suffix = str(config.count)
