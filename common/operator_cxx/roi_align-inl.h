...
class ROIAlignOp : public Operator {
 public:
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    ROIAlignForward(out, data, bbox, param_.sample_per_part, param_.spatial_scale);
  }
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    if (kAddTo == req[roialign::kData] || kWriteTo == req[roialign::kData]) {
      ...
      ROIAlignBackward(grad_in, grad_out, bbox, param_.sample_per_part, param_.spatial_scale);
    }
