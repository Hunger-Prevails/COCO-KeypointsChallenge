...
struct DeformablePSROIPoolingParam : public dmlc::Parameter<DeformablePSROIPoolingParam> {
  ...
  DMLC_DECLARE_PARAMETER(DeformablePSROIPoolingParam) {
    ...
    DMLC_DECLARE_FIELD(group_size).set_default(1).describe("fix group size");
    DMLC_DECLARE_FIELD(pooled_size).set_expect_ndim(2).enforce_nonzero().describe("fix pooled size");
    DMLC_DECLARE_FIELD(part_size).set_default(TShape()).describe("fix part size");
 ...
 public:
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    DeformablePSROIPoolForward(out, data, bbox, trans, top_count, param_.no_trans,
      param_.spatial_scale, param_.group_size, param_.part_size[0], 
      param_.part_size[1], param_.sample_per_part, param_.trans_std);
  }
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    DeformablePSROIPoolBackwardAcc(grad_in, grad_trans, grad_out, data, bbox, trans,
      top_count, param_.no_trans, param_.spatial_scale, param_.group_size,
      param_.part_size[0], param_.part_size[1], param_.sample_per_part, param_.trans_std);
...
class DeformablePSROIPoolingProp : public OperatorProperty {
 public:
  ...
  bool InferShape(std::vector<TShape> *in_shape,
    ...
    CHECK_EQ(dshape[1] % (param_.group_size * param_.group_size), 0) << "incorrect input";
    int output_dim = dshape[1] / (param_.group_size * param_.group_size);

    // out: [num_rois, c, pooled_h, pooled_w]
    // top_count: [num_rois, c, pooled_h, pooled_w]
    out_shape->clear();
    out_shape->push_back(
      Shape4(bshape[0], output_dim, param_.pooled_size[0], param_.pooled_size[1]));
    out_shape->push_back(
      Shape4(bshape[0], output_dim, param_.pooled_size[0], param_.pooled_size[1]));
    return true;
  }
