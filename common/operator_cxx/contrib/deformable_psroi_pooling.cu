...
namespace cuda {
  ...
  __global__ void DeformablePSROIPoolForwardKernel(
    ...
    CUDA_KERNEL_LOOP(index, count) {
      ...
      DType trans_x = no_trans ? static_cast<DType>(0) :
        bottom_trans[(((n * num_classes + class_id) * 2) * part_height + part_h) * part_width + part_w] * trans_std;
      DType trans_y = no_trans ? static_cast<DType>(0) :
        bottom_trans[(((n * num_classes + class_id) * 2 + 1) * part_height + part_h) * part_width + part_w] * trans_std;
  ...
  inline void DeformablePSROIPoolForward(const Tensor<gpu, 4, DType> &out,
    ...
    const int output_dim = out.size(1);
    ...
    DeformablePSROIPoolForwardKernel<DType> << <mxnet::op::mxnet_op::cuda_get_num_blocks(count),
      ...
      group_size, part_height, part_width, num_classes, channels_each_class, top_data, top_count_data);
    ...
    CUDA_KERNEL_LOOP(index, count) {
      ...
      for (int ih = 0; ih < sample_per_part; ih++) {
        for (int iw = 0; iw < sample_per_part; iw++) {
          ...
          atomicAdd(bottom_trans_diff + (((n * num_classes + class_id) * 2) * part_height + part_h) * part_width + part_w, diff_x);
          atomicAdd(bottom_trans_diff + (((n * num_classes + class_id) * 2 + 1) * part_height + part_h) * part_width + part_w, diff_y);
        }
      }
    }
  }
  ...
  inline void DeformablePSROIPoolBackwardAcc(const Tensor<gpu, 4, DType> &in_grad,
    ...
    const int output_dim = out_grad.size(1);
    ...
    DeformablePSROIPoolBackwardAccKernel<DType> << <mxnet::op::mxnet_op::cuda_get_num_blocks(count),
      ...
      group_size, part_height, part_width, num_classes, channels_each_class);
    DeformablePSROIPOOLING_CUDA_CHECK(cudaPeekAtLastError());
  }
  ...
  inline void DeformablePSROIPoolForward(const Tensor<gpu, 4, DType> &out,
    ...
    cuda::DeformablePSROIPoolForward(out, data, bbox, trans, top_count, no_trans, spatial_scale,
      group_size, part_height, part_width, sample_per_part, trans_std);
  }
  ...
  inline void DeformablePSROIPoolBackwardAcc(const Tensor<gpu, 4, DType> &in_grad,
    ...
    cuda::DeformablePSROIPoolBackwardAcc(in_grad, trans_grad, out_grad, data, bbox, trans,
      top_count, no_trans, spatial_scale, group_size, part_height, part_width,
