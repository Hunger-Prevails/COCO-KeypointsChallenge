...
config.TRAIN.solver.lr_step = [16, 20]
config.TRAIN.solver.num_epoch = 24
...
# net params
config = add_network_params(config, 'resnet#34')
config.network.task_type = 'kps'
config.network.image_stride = 32
config.network.sym = 'keypoints.symbols.sym_kps_data.get_symbol'
config.network.sym_body = 'keypoints.symbols.sym_kps_data.get_conv_feat_res4'
...
# exp param
config.exp = 'resnet34_res4_s16_one_loss'
...
if is_debug:
    ...
    config.TRAIN.gpus = '0,1'
    config.TEST.gpus = '0,1'
    config.TRAIN.use_prefetchiter = False
    config.TRAIN.filter_strategy.max_num_images = 50
    config.TEST.filter_strategy.max_num_images = 50
