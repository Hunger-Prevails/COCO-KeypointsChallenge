...
config.TRAIN.image_batch_size = 16
...
# net params
config = add_network_params(config, 'resnet_v1#18')
...
config.network.sym_rcnn_head = 'fpn.symbols.sym_rcnn.get_rcnn_subnet_small'

# det params
if 'fpn' in config.network.task_type:
    config = add_fpn_params(config, is_big_net=False)
elif 'frcnn' in config.network.task_type:
    config = add_frcnn_params(config, is_big_net=False)
...
# exp param
config.exp = 'sym_det_e2e_fpn_c2_exp_resnet18v1'
