...
from common.processing.nms import gpu_nms_wrapper
...
class ProposalROISFPNOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        for s in self._feat_stride_fpn:
            ...
            anchors = expand_anchors(self._anchors_fpn['stride%s' % s].astype(np.float32), height, width, stride)
...
class ProposalROISFPNProp(mx.operator.CustomOpProp):
    ...
    def infer_shape(self, in_shape):
        ...
        rois_shape = (self._rpn_post_nms_top_n, 5)
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return ProposalROISFPNOperator(self._feat_stride_fpn, self._scales, self._ratios, self._output_score,
...
def proposal_rois_fpn(im_info, config, is_train, **kwargs):
    ...
    proposal_method = config.network.proposal_method
    if proposal_method == 'float':
        ...
        rois = mx.contrib.sym.ProposalFPN(im_info=im_info,
                                          ...
                                          threshold=threshold,
                                          **kwargs)
    elif proposal_method == 'standard' or proposal_method == 'x2':
        from common.x2.dpp import dpp
        rois = dpp(im_info=im_info,
                   feature_strides=config.network.rpn_feat_stride,
                   ...
                   dpp_style=proposal_method,
                   out_style=config.network.proposal_out_style,
                   **kwargs)
    else:
        assert False
